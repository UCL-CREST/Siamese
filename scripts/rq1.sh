...
printf "parseMode=$k\n" >> $iconfig
printf "cloneClusterFile=$l\n" >> $iconfig
mvn exec:java -Dexec.mainClass=crest.siamese.experiment.Experiment 2> /dev/null | grep "ARP"
...
printf "parseMode=$k\n" >> $iconfig
printf "cloneClusterFile=$l\n" >> $iconfig
mvn exec:java -Dexec.mainClass=crest.siamese.experiment.Experiment 2> /dev/null | grep "MAP"
# create a temp config (MR-QR)
echo "MR-QR"
